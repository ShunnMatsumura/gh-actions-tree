...
**第2段：DOMフォールバック。** トークン未設定のプライベートリポジトリなどで API が使えない場合は、サイドバーの「Show more workflows」ボタンを増分がなくなるまで自動クリックし、出揃った DOM から一覧をスクレイピングします。ユーザーのセッション Cookie で表示されているページをそのまま読むので、追加の認証が要りません。
